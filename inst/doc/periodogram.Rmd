...
We also suggest setting the `trace=TRUE` option to produce updates.
...
If there was some moon-related pattern of space use and given how we specified our prototype, we would have had a non-zero value as the first `harmonic` value.
We also see that circulation was selected.
...
* `area` is the area of the variance ellipse. It quantifies the amplitude of the stochastic component of the movement process.
* `tau position` is the position autocorrelation time. It is interpreted as the home range crossing time.
* `tau velocity` is the velocity autocorrelation time. It quantifies the stability of the animal`s direction and speed.
...
* `speed` is the expected speed of the movement. For Gamba, there is huge uncertainty in this estimate, but for other maned wolf timeseries, this paramter is better resolved.
