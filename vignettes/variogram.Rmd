...
`ctmm` requires that your dataframe conforms to [Movebank naming conventions](http://rstudio.com) (see `help(as.telemetry)`).
...
The asymptote of our variogram is around 23 square km, and the fact that it takes roughly 20 days for the variogram to asymptote is indicative of the fact that the buffalo's location appears continuous at this timescale. This is also, roughly, the time it takes for the buffalo to cross its home range.

In the next sections we will "fit" models the the variograms.
...
where for both the models `m.iid` and `m.ou`, `sigma` is the asymptotic variance. In the Ornstein-Uhlenbeck (OU) model `m.ou`, `tau` is a single timescale that governs the autocorrelation in position and dictates the animal's home-range crossing time. The independent and identically distributed (IID) null model `m.iid` has no autocorrelation. Notice that all units are in meters and seconds.
...
Also, see `help(uere)` if you have the benefit of calibration data.
