...
The first step to handling errors is to quantify them. Make sure that your data's "dilution of precision" (DOP) and error columns import correctly into `ctmm`. In the following wood turtle dataset, we have some calibration data and a turtle track. Note that the calibration data must be collected from the same model device as the tracking data.

```{r}
data(turtle)
names(turtle[[1]]) # data are not yet calibrated, but HDOP is present
...
```
We can see that the HDOP values yield the better error model,
